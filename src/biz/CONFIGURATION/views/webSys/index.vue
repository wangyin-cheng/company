...
<template>
  ...
  <div class="web-pages web-sys" :class="style.sys">
        ...
        <div class="notice-bar" :class="style.notice">
          <div class="notice-lf">
            ...
            <div class="notice-txt">
              已选择
              <span class="notice-num">{{ userNum }}</span
              >项
            </div>
          </div>
        </div> 
      ...
      <h-tab-pane label="部门管理" name="tab2">
        ...
        <div class="notice-bar" :class="style.notice">
          <div class="notice-lf">
            ...
            <div class="notice-txt">
              已选择
              <span class="notice-num">{{ departmentCountNum }}</span
              >项
            </div>
          </div>
        </div>
      ...
      <h-tab-pane label="环境配置" name="tab3">
        ...
        <div class="environment">
          <h-radio-group v-model="checkEnvironment">
            <h-radio
              :label="env"
              v-for="(env, i) in environmentTitle"
              :key="i"
            ></h-radio>
          </h-radio-group>
        </div>
    ...
    <h-msgBox
      ...
      @on-close="sysCancel"
    >
      <p slot="header" class="user-head">
        {{ popType === "new" ? "新建" : "编辑" }}{{ editTitle[tabIndex] }}
      </p>
      <div class="edit-user">
        ...
        >
          ...
          <h-form-item label="部门id" prop="id" required>
            <h-input
              v-model="departmentItem.id"
              placeholder="请输入部门名称首个小字母"
            ></h-input>
          </h-form-item>
    ...
    :popTxt="delName" :deleteNum="departmentDelNum"
...
export default {
  ...
  methods: {
    ...
    departmentEditFunc(i) {
      this.popType = "eidt";
      this.departmentItem.id = i.row.id || "";
      this.departmentItem.name = i.row.name || "";
      this.editOpen();
    },
    // select删除
    ...
    delMore() {
      ...
      if (this.departmentDelNum === 1) {
        this.delName = this.departmentDeleteData[0].colonyName || 'test'; // 集群名称（字段待确认）
        this.secondExtend = '这条记录，删除后无法恢复，你还要继续吗？';
      } else if (this.departmentDelNum > 1) {
        this.delName = '请确认是否批量删除该部门信息，共' + this.departmentCountNum + '条?';
      }
    },
     // 继续/确认
    confirmFunc() {
    ...
    confirmDelFunc(type) {
      ...
      if (this.departmentDelNum === 1) {
        ...
        this.delName = '';
        this.secondExtend = type === 'success' ? '批量删除成功' : '批量删除失败'; // 具体文案待定
      }
    },
    // 删除，取消
    popCancelFunc() {
